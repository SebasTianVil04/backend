...
class Categoria(AuditoriaMixin, Base):
    __tablename__ = "categorias"

    id = Column(Integer, primary_key=True, index=True)
    nombre = Column(String(100), nullable=False, unique=True, index=True)
    tipo_id = Column(
        Integer, ForeignKey("tipos_categoria.id"), nullable=False, index=True
    )
    modelo_ia_id = Column(
        Integer,
        ForeignKey("modelos_ia.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    ...
    lecciones = relationship(
        "Leccion",
        back_populates="categoria_rel",
        cascade="all, delete-orphan",
        order_by="Leccion.orden",
    )
    dataset_categoria = relationship(
        "CategoriaDataset", back_populates="categoria_rel", uselist=False
    )
    modelo_ia = relationship(
        "ModeloIA", foreign_keys=[modelo_ia_id], backref="categorias_asignadas"
    )

    def __repr__(self):
        return (
            f"<Categoria(id={self.id}, nombre='{self.nombre}', "
            f"modelo_ia_id={self.modelo_ia_id})>"
        )
