...
class Usuario(Base):
    ...
    # CORRECCIÓN: Cambiar nombres de relaciones para que coincidan
    progresos_clase = relationship("ProgresoClase", back_populates="usuario", cascade="all, delete-orphan")
    progresos_leccion = relationship("ProgresoLeccion", back_populates="usuario", cascade="all, delete-orphan")
    ...
    def racha_actual(self):
        """Calcula la racha actual basada en la última práctica"""
        if not self.ultima_practica:
            return 0
        
        # Lógica para calcular racha (esto es un ejemplo simple)
