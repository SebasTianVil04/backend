    ...
    try:
        ...
        for i, frame in enumerate(frames):
            try:
                ...
                gray_actual = cv2.resize(gray, (100, 100))
                if gray_anterior is not None:
                    diff = cv2.absdiff(gray_actual, gray_anterior)
                    variaciones.append(diff.mean())
                gray_anterior = gray_actual
        ...
        estabilidad_ok = True
        umbral_estabilidad = 35 if tipo_sena == 'ESTATICA' else 55
        if variaciones:
            ...
            if config['requiere_estabilidad']:
                estabilidad_ok = estabilidad_promedio < umbral_estabilidad
